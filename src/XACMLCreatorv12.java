...
public class XACMLCreatorv12{
	public String xmlFile="<?xml version="+"\""+"1.0"+"\""+" encoding="+"\""+"UTF-8"+"\""+" standalone="+"\""+"yes"+"\""+"?>\n"; 
	Mapping map;
	XACMLCreatorv12(ModelPopulatorOAM mp, String outputPolicyFile){
		String xacmlClass;
		...
		for (int appCount=0;appCount<apps.size();appCount++) {
			...
			for (int polCount=0;polCount<pols.size();polCount++) {
				...
				String policyDesc="<Target>\n"+targetStrSub+classDescURL+targetStrAct+appStr+"</Target>\n"+metadataDesc+"<Rule Effect="+"\""+"Permit"+"\""+" RuleId="+"\""+"null:permit-rule"+"\""+">\n"+ruleStrAct+ruleStrSub+"</Rule>\n</Policy>\n";
				xmlFile=xmlFile+policyDesc;
			}
		}
		xmlFile=xmlFile+"</PolicySet>\n";
		try {
			out.write(xmlFile);
			out.flush();
			out.close();
	...
	public String createApplicationClassTarget(String appName){
        ...
        classDesc=classDesc+"<AllOf>\n<Match MatchId="+"\""+matchID+"\""+">\n";
        classDesc=classDesc+"<AttributeValue DataType="+"\""+datatype+"\""+">"+appName+"</AttributeValue>\n";
        classDesc=classDesc+"<AttributeDesignator MustBePresent="+"\""+false+"\""+" DataType="+"\""+datatype+"\""+" AttributeId="+"\""+attributeID+"\""+" Category="+"\""+category+"\""+"/>\n";
        classDesc=classDesc+"</Match>\n</AllOf>\n";
		return classDesc;
	}
	public String createURLClass(URL url, String appName){	
        ...
        classDesc=classDesc+"<AllOf>\n<Match MatchId="+"\""+matchID+"\""+">\n";
        classDesc=classDesc+"<AttributeValue DataType="+"\""+datatype+"\""+">"+url.geturl()+"</AttributeValue>\n";
        classDesc=classDesc+"<AttributeDesignator MustBePresent="+"\""+true+"\""+" DataType="+"\""+datatype+"\""+" AttributeId="+"\""+attributeID+"\""+" Category="+"\""+category+"\""+"/>\n";
        classDesc=classDesc+"</Match>\n</AllOf>\n";
		return classDesc;
	}
	public String createActionClassTarget(Action ac, String appName){	
		...
		targetStr=targetStr+"<AllOf>\n<Match MatchId="+"\""+matchID+"\""+">\n";
		targetStr=targetStr+"<AttributeValue DataType="+"\""+datatype+"\""+">"+ac.getactionType()+"</AttributeValue>\n";
		targetStr=targetStr+"<AttributeDesignator MustBePresent="+"\""+"true"+"\""+" DataType="+"\""+datatype+"\""+" AttributeId="+"\""+attributeID+"\""+" Category="+"\""+category+"\""+"/>\n";
		targetStr=targetStr+"</Match>\n</AllOf>\n";
		return targetStr;
	}
	public String createActionClassRule(Action ac, String appName){
        ...
        String attributeID="urn:oasis:names:tc:xacml:1.0:action:action-id";
        String category="urn:oasis:names:tc:xacml:3.0:attribute-category:action";
		ruleStr=ruleStr+"<AllOf>\n<Match MatchId="+"\""+matchID+"\""+">\n";
		ruleStr=ruleStr+"<AttributeValue DataType="+"\""+datatype+"\""+">"+ac.getactionType()+"</AttributeValue>\n";
		ruleStr=ruleStr+"<AttributeDesignator MustBePresent="+"\""+"true"+"\""+" DataType="+"\""+datatype+"\""+" AttributeId="+"\""+attributeID+"\""+" Category="+"\""+category+"\""+"/>\n";
		ruleStr=ruleStr+"</Match>\n</AllOf>\n";
		return ruleStr;
	}
	public String createSubjectClassTarget(Subject sub){
		...
		//       1. Any body who is authenticated can access
		//       2. Any body who is authenticated to a realm will have access
		//       3. Any body who is part of a user/group will have access
		// 
		...
		for (int authCount=0;authCount<auths.size();authCount++) {	
			...
			if (auth.getauthType().equals("NoSubject")){ // The resource's protection level is 'EXCLUDED'
				...
				targetStr=targetStr+"<AllOf>\n<Match MatchId="+"\""+matchID+"\""+">\n";
				targetStr=targetStr+"<AttributeValue DataType="+"\""+datatype+"\""+">nosubject</AttributeValue>\n";
				targetStr=targetStr+"<AttributeDesignator MustBePresent="+"\""+"true"+"\""+" DataType="+"\""+datatype+"\"";
				...
				targetStr=targetStr+"<AllOf>\n<Match MatchId="+"\""+matchID+"\""+">\n";
				targetStr=targetStr+"<AttributeValue DataType="+"\""+datatype+"\""+">{}</AttributeValue>\n";
				targetStr=targetStr+"<AttributeDesignator MustBePresent="+"\""+"true"+"\""+" DataType="+"\""+datatype+"\"";
				targetStr=targetStr+" AttributeId="+"\""+attributeID+"\"";
				targetStr=targetStr+" Category="+"\""+category+"\""+"/>";
				targetStr=targetStr+"</Match>\n</AllOf>\n";
		...
		for (int authzCount=0;authzCount<authzs.size();authzCount++) {
			if (authzs!=null) {
				...
				String category="urn:oasis:names:tc:xacml:1.0:subject-category:access-subject";
				targetStr=targetStr+"<AllOf>\n<Match MatchId="+"\""+matchID+"\""+">\n";
				targetStr=targetStr+"<AttributeValue DataType="+"\""+datatype+"\""+">"+authz.getelementName()+"</AttributeValue>\n";
				targetStr=targetStr+"<AttributeDesignator MustBePresent="+"\""+"true"+"\"" + " DataType="+"\""+datatype+"\"";
				targetStr=targetStr+" AttributeId="+"\""+attributeID+"\"";
				targetStr=targetStr+" Category="+"\""+category+"\""+"/>";
				targetStr=targetStr+"</Match>\n</AllOf>\n";
	...
	public String createSubjectClassRule(Subject sub, Vector<ConstraintGroup> constGroup){		
		...
		for (int authCount=0;authCount<auths.size();authCount++) {	
			if (auths!=null) { // This check is redundant, but will keep it for now
				...
				attrValString=attrValString+"<AttributeValue DataType="+"\""+map.datatypeSubject.get(auth.getauthType())+"\""+" privilegeComponent="+"\""+privComponent+"\""+">"+value+"</AttributeValue>\n";
			}
		}
		for (int authzCount=0;authzCount<authzs.size();authzCount++) {	
			if (authzs!=null) {
				Authorization authz=authzs.elementAt(authzCount);
				//String privComponent;
				String attributeID="urn:sun:opensso:entitlement:json-subject";
				String category="urn:oasis:names:tc:xacml:1.0:subject-category:access-subject";
				if (authz.getelementType()=="IdentitySubject") {
					privComponent="entitlementSubject";
					attrValString=attrValString+"<AttributeValue DataType="+"\""+map.datatypeSubject.get(authz.getelementType())+"\""+" privilegeComponent="+"\""+privComponent+"\""+">"+authz.getelementName()+"</AttributeValue>\n";
				}
				else {
					privComponent="entitlementCondition";
					attrValString=attrValString+"<AttributeValue DataType="+"\""+map.individualdatatypeCondition.get(authz.getelementType())+"\""+" privilegeComponent="+"\""+privComponent+"\""+">"+authz.getelementName()+"</AttributeValue>\n";
				}		
			}
		}
		...
		for (int groupCount=0;groupCount<constGroup.size();groupCount++) {
		    ...
		    if (constGroup.size()>1) {
		    	if (permittype.equals("DENY")) {
		    		...
		    		//String denyInitial = prefix1 + "\""+"className"+prefix1+"\""+":"+ prefix1+ "\""+ "SAMPLE" + prefix1+"\""+", "+ prefix1 +"\""+ "state" +prefix1 +"\""+":"+prefix1 +"\""+"{"+ prefix2 +"\""+ "memberECondition"+ prefix2+"\""+":"+"{";
		    		if (totalConditions>1){
		...
		if (conditions.size()==1) {
			...
			attrValString = "<AttributeValue DataType="+"\""+ultimateDatatype+"\""+ " privilegeComponent="+"\""+privComponent+"\""+">"+Util.encodeXmlAttribute(body)+"</AttributeValue>\n";
		}
		else {
		...
		//attrValString = "<AttributeValue DataType="+"\""+datatype+"\""+ " privilegeComponent="+"\""+privComponent+"\""+">"+Util.encodeXmlAttribute(body)+"</AttributeValue>\n";
		// Now create the the full ruleStr
		ruleStr=ruleStr+"<Condition>\n<Apply FunctionId="+"\""+functionID+"\""+">\n";
		ruleStr=ruleStr+attrValString;
		ruleStr=ruleStr+"</Apply>\n</Condition>\n";
		return ruleStr;
	}
    ...
    public String createXACMLClassPolicySet(String indivClass){
		...
		classDesc=classDesc+"<"+indivClass+" ";
	...
	public String createXACMLClass(Object obj, String classType){
		...
		for (int i = 0; i < classNames.length; i++) {
			...
			for (int j = indivClasses.length-1; j >= 0; j--) {
				xmlFile=xmlFile+"<"+indivClasses[j]+" ";
				classDesc=classDesc+"<"+indivClasses[j]+" ";
	...
	public String createMetaData(String appName){
		...
		String varDefStartTag="<VariableDefinition VariableId";
		String varDefEndTag="</VariableDefinition>";
		String attrDefStartTag="<AttributeValue DataType";
		String attrDefEndTag="</AttributeValue>";
